...
def binary_search(my_list: Union[ArrayList[T], ArrayR], target_item: T) -> int:
    """
    ...
    Args:
        my_list (Union[ArrayList[T], ArrayR]): the list to be searched.
    ...
    """
    def _binary_search_aux(my_list: Union[ArrayList[T], ArrayR[T]], target_item: T, lo: int, hi: int) -> int:
        """
        Auxiliary method used by binary search.

        Args:
            my_list (Union[ArrayList[T], ArrayR[T]]): The list we wish to search.
            target_item (T): the target element to be found.
